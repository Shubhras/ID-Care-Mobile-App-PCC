import { Image, View, Text, ScrollView, TouchableOpacity, ActivityIndicator, Alert, Pressable } from "react-native" // Added ScrollView, ActivityIndicator
...
import { SCREEN_HEIGHT, STANDARD_SPACING } from "../../constants/Constants"
import PatientPickerModal from "../../components/modls/PatientPickerModal"
import { useEffect, useState } from "react"
import CustomSafeAreaView from "../../components/global/CustomSafeAreaView"
import { GetCultureSource, GetDoctors, GetInitialDx, GetInitialRx, GetmdSteward, Getorganism, GetPrecautions, GetUpdatePatientsDetails, patientDetails } from "../../auth/auth"
...
const EditPatientDetail = ({ navigation, route }) => {
    ...
    const [abxChecklistModalVisible, setAbxChecklistModalVisible] = useState(false);
    const [selectedAbxChecklist, setSelectedAbxChecklist] = useState({ name: 'N/A' });
    ...
    const [cultureSourceModalVisible, setCultureSourceModalVisible] = useState(false);
    ...
    const openLink = async () => {
        try {
            console.log('Opening form URL:iiiiiiiiiiiiiii', selectedFormUrl?.formUrls);
    ...
    const getPatientDetails = async () => {
        ...
        try {
            ...
            if (response.status === 1) {
                ...
                setSelectedInitialDx({ id: records?.initial_dx, name: records.initial_dx_name || '' })
                setSelectedInitialRx({ id: records?.initial_rx, name: records.initial_rx_name || '' })
                setSelectedAbxChecklist({ name: records?.infection_surveillance_checklist || 'N/A' })
                ...
                setSelectedDoctor({ id: records?.doctor_id, name: records?.doctor_name || '' })
                setSelectedMDSteward({ id: records?.md_steward_id, name: records?.md_steward || '' })
                ...
                <ScrollView style={styles.scrollView}
                    ...
                    <View style={styles.listContainer}>
                        <View style={styles.rowIdWrapper}>
                            ...
                            <CustomText style={[styles.valueText, { color: Colors.boysenberry, }]}>{patient_id}</CustomText>
                        </View>
                        <View style={styles.hedingWrapper}>
                            <Text style={styles.hedingText}>Infection Onset</Text>
                        </View>
                        <View style={styles.rowStatusWrapper}>
                            <Pressable
                                onPress={() => { setFacility('Hospital') }}
                                style={[styles.statusButton, { backgroundColor: facility == 'Hospital' ? Colors.boysenberry : Colors.inputBackgroundColor }]}>
                                <CustomText style={[styles.titleText, { color: facility == 'Hospital' ? Colors.white : Colors.textLowContrast }]}>Hospital/CAI</CustomText>
                            </Pressable>
                            <Pressable
                                onPress={() => { setFacility('Facility') }}
                                style={[styles.statusButtonRight, { backgroundColor: facility == 'Facility' ? Colors.boysenberry : Colors.inputBackgroundColor }]}>
                                <CustomText style={[styles.titleText, { color: facility == 'Facility' ? Colors.white : Colors.textLowContrast }]}>Facility/HAI</CustomText>
                            </Pressable>
                        </View>

                        <View style={styles.hedingWrapper}>
                            <Text style={styles.hedingText}>Care Unit Name</Text>
                        </View>
                        <View style={styles.rowWrapper}>
                            <CustomText style={[styles.valueText, { color: Colors.boysenberry, }]}>{patientDetail?.care_unit_name}</CustomText>
                        </View>

                        <View style={styles.hedingWrapper}>
                            <Text style={styles.hedingText}>Initial Dx</Text>
                        </View>
                        <Pressable style={styles.rowWrapper}
                        ...
                        <View style={styles.hedingWrapper}>
                            <Text style={styles.hedingText}>Initial Rx</Text>
                        </View>
                        <Pressable style={styles.rowWrapper} onPress={() => setInitialRxModalVisible(true)}>
                            <CustomText style={styles.valueText} numberOfLines={1} >
                        ...
                        <View style={styles.hedingWrapper}>
                            <Text style={styles.hedingText}>Initial DOT</Text>
                        </View>
                        <View style={styles.rowWrapper}>
                            ...
                            <View style={styles.counterButtonView}>
                                ...
                                <CustomText style={[styles.numberDotText, { color: Colors.black }]}>
                                    {initialDotNumber}
                                </CustomText>
                        ...
                        <View style={styles.hedingWrapper}>
                            <Text style={styles.hedingText}>Abx Checklist</Text>
                        </View>
                        <Pressable style={styles.rowWrapper} onPress={() => setAbxChecklistModalVisible(true)}>
                            <CustomText style={styles.valueText} numberOfLines={1} >
                        ...
                        {(selectedAbxChecklist?.name !== 'N/A' && editAbxChecklist == 'search') &&
                            <>
                                <Pressable style={styles.formButtonWrapper} onPress={openLink}>
                                    <CustomText style={[styles.hedingText, { color: Colors.white }]}>
                                        Print Abx Checklist form
                                    </CustomText>
                                </Pressable>
                                <View style={styles.rowWrapperOption}>
                                    <CustomText style={styles.hedingText}>Criteria Met : </CustomText>
                                    <Pressable style={styles.optionRow} onPress={() => { setCriteriaMetOption('Yes') }}>
                                        <Icons name={criteriaMetOption == 'Yes' ? "check-circle" : "circle-thin"} size={24} color={Colors.deepPurple} iconType="FontAwesome" />
                                        <CustomText style={[styles.valueText, { color: Colors.black }]}>Yes</CustomText>
                                    </Pressable>
                                    <Pressable style={styles.optionRow} onPress={() => { setCriteriaMetOption('No') }}>
                                        <Icons name={criteriaMetOption == 'No' ? "check-circle" : "circle-thin"} size={24} color={Colors.deepPurple} iconType="FontAwesome" />
                                        <CustomText style={[styles.valueText, { color: Colors.black }]}>No</CustomText>
                                    </Pressable>
                                </View>
                            </>}

                        <View style={styles.hedingWrapper}>
                            <Text style={styles.hedingText}>Organism</Text>
                        </View>

                        <Pressable style={styles.rowWrapper} onPress={() => setOrganismModalVisible(true)}>
                            <CustomText style={styles.valueText} numberOfLines={1} >
                        ...
                        <View style={styles.hedingWrapper}>
                            <Text style={styles.hedingText}>Precautions</Text>
                        </View>
                        <Pressable style={styles.rowWrapper} onPress={() => setPrecautionsModalVisible(true)}>
                            <CustomText style={styles.valueText} numberOfLines={1} >
                        ...
                        <View style={styles.hedingWrapper}>
                            <Text style={styles.hedingText}>Culture Source</Text>
                        </View>
                        <Pressable style={styles.rowWrapper} onPress={() => setCultureSourceModalVisible(true)}>
                            <CustomText style={styles.valueText} numberOfLines={1} >
                        ...
                        <View style={styles.hedingWrapper}>
                            <Text style={styles.hedingText}>Doctor</Text>
                        </View>
                        <Pressable style={styles.rowWrapper} onPress={() => setDoctorModalVisible(true)}>
                            <CustomText style={styles.valueText} numberOfLines={1} >
                        ...
                        <View style={styles.hedingWrapper}>
                            <Text style={styles.hedingText}>MD Steward</Text>
                        </View>
                        <Pressable style={styles.rowWrapper} onPress={() => setMDStewardModalVisible(true)}>
                            <CustomText style={styles.valueText} numberOfLines={1} >
                        ...
                        <View style={styles.hedingWrapper}>
                            <Text style={styles.hedingText}>Date of Starting ABX</Text>
                        </View>
                        <Pressable style={[styles.rowWrapper, { marginBottom: STANDARD_SPACING * 1.5 }]}
                            onPress={() => setPickerVisible(true)}>
                             <CustomText style={styles.valueText}>{date ? String(date) : 'Date Of Starting ABX'}</CustomText>

                        </Pressable>
                        <View style={[styles.rowWrapper, { marginBottom: STANDARD_SPACING * 1.5 }]}>
                            <CustomText style={styles.hedingText}>MD Steward Response</CustomText>
                            {patientDetail?.md_stayward_response == 'Agree' && <CustomText style={[styles.valueText, { color: 'green' }]}>{patientDetail?.md_stayward_response}</CustomText>}
                            {patientDetail?.md_stayward_response == 'NoResponse' && <CustomText style={[styles.valueText, { color: 'blue' }]}>{patientDetail?.md_stayward_response}</CustomText>}
                            {patientDetail?.md_stayward_response == 'Disagree' && <CustomText style={[styles.valueText, { color: 'red' }]}>{patientDetail?.md_stayward_response}</CustomText>}
                            {patientDetail?.md_stayward_response == 'Modify' && <CustomText style={[styles.valueText, { color: 'orange' }]}>{patientDetail?.md_stayward_response}</CustomText>}
                        </View>
